...
%%handle_cast({update_position, Picture_Name, New_Pos, New_Mov}, Nodes) ->
%%  graphics ! {update_position, Picture_Name, New_Pos, New_Mov},
%%  {noreply, Nodes};

handle_cast({generate_position, Picture_Name, PosX, PosY}, Nodes) ->
  global:send(graphics, {generate_position, Picture_Name, PosX, PosY}),
  %graphics ! {generate_position, Picture_Name, PosX, PosY},
  {noreply, Nodes};
...
graphics_process(Frame) ->
  graphics_message(),
  graphics_update_messages(10),collision_detect
...
collision_detect(Picture_Line, Picture_To_Campre) ->
  ...
  case Flag of
    ...
    true  ->
      ...
      receive after ?PIC_PROCESS_TIMEOUT -> ok end
  end.
